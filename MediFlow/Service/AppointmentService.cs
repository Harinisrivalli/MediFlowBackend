...
namespace MediFlow.Service
{
    public class AppointmentService
    {
            ...
            DTO.AppointmentDTO appointment)
        {
            ...
            if (data != null)
            {
                if(data.Status == "Cancelled Successfully")
                {
                    return new DTO.AppointmentDTOResponse
                    {
                        Reason = data.Status
        ...
        public async Task<List<DTO.AppointmentDTOResponse>> GetDoctorAppointments(int id, DateTime date)
        {
            var status = await _appointmentRepo.GetDoctorAppointments(id, date);
            if(status != null)
            {
                List<DTO.AppointmentDTOResponse> list = status.Select(obj => new DTO.AppointmentDTOResponse
                {
                    Id = obj.Id,
                    PatientId = obj.PatientId,
                    DoctorId = obj.DoctorId,
        ...
        public async Task<List<DTO.AppointmentDTOResponse>> GetPatientAppointments(int id, DateTime date)
        {
            var status = await _appointmentRepo.GetPatientAppointments(id, date);
            if (status == null)
                return null;
            List<DTO.AppointmentDTOResponse> list = status.Select(obj => new DTO.AppointmentDTOResponse
            {
